...
def get_fluentbit_process():
    """
    API endpoint for get fluentbit status
    :return:
    """
    return {FLUENTD_SERVICE: get_fluentbit_status()}
...
def set_fluentbit_config(logging):
    """
    API endpoint for configuring logging config
    :param logging:
    :return:
    ...
    change_fluentbit_status(STOP)
    if logging:
        obj = fluentbit_manager.FluentbitPluginManager(logging)
        return_dict = obj.set_config()
    else:
        delete_fluentbit_config()
    if ERROR not in return_dict:
        return_dict[FLUENTBIT_STATUS] = get_fluentbit_process()
    return return_dict
...
def get_fluentbit_config():
    exsting_data = file_reader(FluentbitData)
    if exsting_data:
        exsting_data = json.loads(exsting_data)
        exsting_data[FLUENTBIT_STATUS] = get_fluentbit_process()
    else:
        exsting_data = {}
    return exsting_data
...
def get_supported_fb_logging_plugins():     
    mapping_list = get_fluentbit_plugins_mapping()      
    result = []     
    for key in mapping_list:        
        result.append(key)      
    return result
...
def get_logging_fb_plugins_params(plugins=None):
    if plugins is None:
        plugins = []
    result = {PLUGINS: []}
    for i in get_supported_fb_logging_plugins():
        if plugins and i not in plugins:
            continue
        result[PLUGINS].append({NAME: i, "config": {"components": [{"name": "Name of the component", "filter": {}}]}})
...
def write_config_to_target(es_config, interval=CONFIG_WRITE_INTERVAL):
    ...
    if host and port and index:
        ...
        data[FB_LOGGING] = get_fluentbit_config()
...
def get_target_status():
    ...
    try:
        ...
        exsting_fb_data = file_reader(FluentbitData)
        fluentbit_exsting_data = json.loads(exsting_fb_data)
        if fluentbit_exsting_data:
            for target in fluentbit_exsting_data['targets']:
                target_details = {}
                target_details["name"] = target["name"]
                target_details["index"] = target["index"]
                es_username = ''
            ...
            logger.error("No workload data found in fluentbit_data.json")
