...
def change_fluentbit_status(oper):
    logger.info("Change td-agent-bit status, operation %s", oper)
    service = "td-agent-bit"
    status = get_service_status(service)
    if status == -1:
        logger.warning("td-agent-bit service not found, fluentd not installed")
    elif oper == "start":
        pid = get_process_id(service)
        if pid > 0:
            restart_service(service)
...
def get_fluentbit_plugins_mapping():        
    dirname, filename = os.path.split(os.path.abspath(__file__))        
    file_name = os.path.join(dirname, FluentbitPluginMappingFilePath)       
    return read_yaml_file(file_name)

def get_fluentbit_config():     
    dirname, filename = os.path.split(os.path.abspath(__file__))        
    file_name = os.path.join(dirname, FluentbitConfigurationFilePath)       
    return read_yaml_file(file_name)
...
def delete_fluentbit_config():
    logger.info("Delete existing fluentbit config")
    error_msg = ''
    fluentbit_conf_dir = FluentbitPluginConfDir + os.path.sep
    try:
        ...
        if PLUGINS in exsting_data:
            for x_plugin in exsting_data[PLUGINS]:
                if NAME in x_plugin:
                    fluentbit_conf_filepath = fluentbit_conf_dir + x_plugin[NAME]
                    command = "rm -rf " + fluentbit_conf_filepath
                    out, err = run_shell_command(command)
                    if err:
                        msg = "Failed to Delete Fluentbit conf file " + fluentbit_conf_filepath
                        logger.warning(msg)

            fluentbit_conf_filepath = fluentbit_conf_dir + "td-agent-bit.conf"
            command = "rm -rf " + fluentbit_conf_filepath
            out, err = run_shell_command(command)
            if err:
                msg = "Failed to Delete Fluentbit conf file " + fluentbit_conf_filepath
                logger.warning(msg)
        logger.info("Successfully deleted old fluentbit config")
        return True, error_msg

    except Exception as e:
        error_msg += "delete fluentbit config: "
        error_msg += str(e)
        logger.error(error_msg)
    return False, error_msg
