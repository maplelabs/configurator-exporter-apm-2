...
urls = (
    ...
    "/api/fluentbit/process", "FluentbitProcess",
    "/api/service", "Service",
    "/api/service/", "Service",
...
class Config:
    def POST(self):
        result = {METRICS: None, LOGGING: [], FB_LOGGING: []}
        try:
        ...
        if not (metrics or logging):
            ...
            result[FB_LOGGING] = configurator.set_fluentbit_config(logging)
    ...
    def GET(self):
        ...
        if len(url_param) == 0:
            ...
            result[FB_LOGGING] = configurator.get_fluentbit_config()
            ...
            result[FB_LOGGING] = configurator.get_supported_fb_logging_plugins()
            ...
            result[FB_LOGGING] = configurator.get_fluentbit_plugins_mapping()
            ...
            if user_data.logging_plugins:
                ...
                result[FB_LOGGING] = configurator.get_logging_fb_plugins_params(logging_plugin_instances)
            if not (user_data.metrics_plugins or user_data.targets or user_data.logging_plugins):
                result[METRICS] = configurator.get_metrics_plugins_params(metric_plugin_instances)
                result[LOGGING] = configurator.get_logging_plugins_params(logging_plugin_instances)
                result[FB_LOGGING] = configurator.get_logging_plugins_params(logging_plugin_instances)
                result[TARGETS] = configurator.get_targets_params(target_instances)
        else:
            error_msg = "URl not Found"
            raise web.notfound(error_msg)
